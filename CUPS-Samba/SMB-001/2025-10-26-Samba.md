...
| Cannot access share          | SELinux or firewall  | Check contexts and port access                        |
...
| SMB Version Control | Enforce minimum protocol (e.g. `min protocol = SMB2`)      |
| User Access Control | Limit access with `valid users` and filesystem permissions |
